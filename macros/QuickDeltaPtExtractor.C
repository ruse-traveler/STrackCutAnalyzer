...
static const Ssiz_t  NTrkCuts = 5;
static const TString SInTrack = "ntp_track";
static const TString SInTruth = "ntp_gtrack";

// default parameters
static const TString SInDef  = "input/embed_only/final_merge/sPhenixG4_run6hijing50khz_embedScanOn_embedOnly.pt020n100pim.d3m4y2023.root";
static const TString SOutDef = "varyDeltaPtCut.beforeSectorFix.pt020n100pim.d26m4y2023.root";
...
void QuickDeltaPtExtractor(const TString sInput = SInDef, const TString sOutput = SOutDef) {
  ...
  const TString sPurityBase       = "Purity";
  ...
  const TString sPurityAxis   = "purity";
  ...
  for (Long64_t iTrk = 0; iTrk < nTrks; iTrk++) {
    ...
    const Bool_t isGoodTrk   = (isInZVtxCut && isInMVtxCut && isInTpcCut && isInPtCut && isInQualCut);
  ...
  // calculate purities
  for (Ssiz_t iCut = 0; iCut < NCuts; iCut++) {
    purity[iCut] = (Double_t) nNorm[iCut] / (Double_t) nWeird[iCut];
  }
  ...
  // announce purities
  cout << "    Finished tuple loops! Calculated purities:" << endl;
  for (Ssiz_t iCut = 0; iCut < NCuts; iCut++) {
    cout << "      n(Norm, Weird) = (" << nNorm[iCut] << ", " << nWeird[iCut] << "), purity = " << purity[iCut] << endl;
  }

  // make purity graph
  TString sPurity("gr");
  sPurity.Append(sPurityBase.Data());

  TGraph *grPurity = new TGraph(NCuts, ptDeltaMax, purity);
  grPurity -> SetName(sPurity.Data());
  cout << "    Made purity graph." << endl; 
  ...
  grPurity       -> SetMarkerColor(fColTrue);
  grPurity       -> SetMarkerStyle(fMarTrue);
  grPurity       -> SetFillColor(fColTrue);
  grPurity       -> SetFillStyle(fFil);
  grPurity       -> SetLineColor(fColTrue);
  grPurity       -> SetLineStyle(fLin);
  grPurity       -> SetLineWidth(fWid);
  grPurity       -> SetTitle(sTitle.Data());
  grPurity       -> GetXaxis() -> SetRangeUser(rDeltaRange[0], rDeltaRange[1]);
  grPurity       -> GetXaxis() -> SetTitle(sDeltaCutAxis.Data());
  grPurity       -> GetXaxis() -> SetTitleFont(fTxt);
  grPurity       -> GetXaxis() -> SetTitleSize(fTit[1]);
  grPurity       -> GetXaxis() -> SetTitleOffset(fOffX[1]);
  grPurity       -> GetXaxis() -> SetLabelFont(fTxt);
  grPurity       -> GetXaxis() -> SetLabelSize(fLab[1]);
  grPurity       -> GetXaxis() -> CenterTitle(fCnt);
  grPurity       -> GetYaxis() -> SetTitle(sPurityAxis.Data());
  grPurity       -> GetYaxis() -> SetTitleFont(fTxt);
  grPurity       -> GetYaxis() -> SetTitleSize(fTit[1]);
  grPurity       -> GetYaxis() -> SetTitleOffset(fOffY[1]);
  grPurity       -> GetYaxis() -> SetLabelFont(fTxt);
  grPurity       -> GetYaxis() -> SetLabelSize(fLab[1]);
  grPurity       -> GetYaxis() -> CenterTitle(fCnt);
  ...
  TCanvas *cPurity = new TCanvas("cPurity", "", width, heightNR);
  cPurity  -> SetGrid(fGrid, fGrid);
  cPurity  -> SetTicks(fTick, fTick);
  cPurity  -> SetBorderMode(fMode);
  cPurity  -> SetBorderSize(fBord);
  cPurity  -> SetFrameBorderMode(fFrame);
  cPurity  -> SetLeftMargin(fMarginL);
  cPurity  -> SetRightMargin(fMarginR);
  cPurity  -> SetTopMargin(fMarginTNR);
  cPurity  -> SetBottomMargin(fMarginBNR);
  cPurity  -> SetLogx(fLogX);
  cPurity  -> SetLogy(fLogYNR);
  cPurity  -> cd();
  grPurity -> Draw("ALP");
  info     -> Draw();
  cuts     -> Draw();
